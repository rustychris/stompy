# -*- coding: utf-8 -*-
"""
Taken from suntanspy: https://github.com/ofringer/suntanspy
...
def tide_pred(modfile,lon,lat,time,z=None,conlist=None):
    ...
    t1992 = (time[0]-base_time)/np.timedelta64(1,'D')
...
def extract_HC(modfile,lon,lat,z=None,conlist=None):
    ...
    if not os.path.exists(hfile):
        # Maybe it has an extra level of DATA in there
        fix=lambda s: s.replace('DATA/DATA','DATA')
        hfile=fix(hfile)
        uvfile=fix(uvfile)
        grdfile=fix(grdfile)
...
def read_OTPS_grd(grdfile):
    ...
    if nob == 0:
       ...
       f.seek(8,1)
       iob = np.fromfile(f,dtype=np.int32,count=2*nob[0])
       iob.byteswap(True)
